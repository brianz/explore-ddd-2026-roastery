...
Team brief: [`docs/student/roastery.md`](../../docs/student/roastery.md). Read it
first — it has the responsibilities and open questions this file doesn't repeat.
...
The shared `sam/roastables.json` catalog is real data (not a hint about your
internal model) — read it for what roastables and roast levels actually exist.

## Do not

- **Do not let a request decide the roast level.** Whatever roast level a roastable
  should be roasted at is a property of the roastable, not a per-request dial —
  Ethiopia is always the same roast level every time it's charged.
- **Do not import from another context's directory.** Not `sam/green-coffee-purchasing/`,
...
- **Do not assume delivery is exactly-once.** EventBridge delivers at-least-once —
  a redelivered event that consumes green coffee twice for one batch is a real
