...
Team brief: [`docs/student/fulfillment.md`](../../docs/student/fulfillment.md). Read
it first — it has the responsibilities and open questions this file doesn't repeat.
...
- **Do not decide whether stock exists.** That's Inventory & Packaging's call. You
  react to their decision; you don't second-guess it.
...
- **Do not assume delivery is exactly-once.** EventBridge delivers at-least-once —
  a redelivered event that ships the same order twice is a real failure mode to
  design against, not a hypothetical.
